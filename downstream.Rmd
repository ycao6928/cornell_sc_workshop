...
**However**, due to the time limit of this workshop, we will only run the `scClust` clustering for `k = 6` as a demonstration and load a saved result with more values of `k` computed. (You don't necessarily need to run the code for `scClust`, but you will need to load the pre-computed results.)

```{r, warning = FALSE, eval = FALSE}
## For demonstration purpose, this is the code for running `scClust` with k = 6 (which is actually the number of cell types in our dataset)
...
```{r, eval = FALSE}
marker_cluster4 = scdney::findMarker(
  mat = mat,
...
fig3 = ggplot(data = tsne_plotdf,
              mapping = aes(x = tsne1, y = tsne2, colour = truth)) +
  geom_point(alpha = 0.5) +
  labs(x = "",
       y = "",
       title = "'True' labels provided by authors")
...
## Proportion estimates through `scDC`


`scDC` is a package developed by the Sydney Precision Bioinformatics Group. It simulates the uncertainty associated with cell-type proportion estimates through bootstrap and confidence interval calculation and provides visualisation of the uncertainty.   

The usage of `scDC` is very simple. The main function `scDC_clustering` requires the following inputs:

+ `exprsMat`:  Single cell xpression matrix
+ `cellTypes`: Cell type label for each cell
+ `subject`: Patient, treatment information
+ `calCI_method`: Confidence interval method. Options include BCa, percentile, and multinom. 
```{r, eval=FALSE}
result <- scDC_clustering(exprsMat =  mat, 
                          cellTypes = lab , 
                          subject = rep("liver", length(lab) ) , 
                          calCI_method= c( "multinom", "percentile"))
```

To save time, we have ran the above function and provided the result below.   
`scDC` contains two visualisation methods, barplot and density plot.
Based on these figures, we can see that instead of a fixed proportion, we obtain a distribution of proportions associated with each cell-type proportion.   

```{r fig.height=4, fig.width=8, message=FALSE, warning=FALSE}
datapath = "./data/"

result <- readRDS(paste0(datapath,"scDC_result.RDS"))
barplotCI(result, "liver")
densityCI(result, "liver")
```

<h4> <span class="glyphicon glyphicon-education" aria-hidden="true"></span> Quick quiz </h4>

1. What is the range of proportion distribution for the cell-type endothelial cell?  
2. Any rare cell type? What is the most common cell type?
